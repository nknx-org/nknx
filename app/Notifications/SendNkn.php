...
class SendNkn extends Notification
{
     ...
     * @return void
     */
    public function __construct($wallet, $amount, $recipient)
    {
        $this->wallet = $wallet;
    ...
    {
        if(isset($notifiable->notificationPreferences['send_nkn'])){
            return $notifiable->notificationPreferences['send_nkn'];
        } else{
            return [];
        }
    }
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ...
            ->subject('You sent out some NKN')
            ->line('Your wallet with the address ' . $this->wallet->address .' just sent out some NKN.')
            ->line('')
            ->line('Amount: ' . $this->amount)
            ->line('To: ' . $this->recipient)
            ->line('')
            ->line('If you are not aware of this transaction please act immediately.');
    }
    ...
    {
        return [
            'header' => "You sent out some NKN",
            'excerpt' => "Your address " . $this->wallet->address ." sent out some NKN.",
            'text' => "
                ...
                <p>Your wallet with the address <b>" . $this->wallet->address ."</b> just sent out some NKN.</p>
                <p><br/>Amount: " . $this->amount ."</p>
                <p>To: " . $this->recipient ."</p>
                <p>If you are not aware of this transaction please act immediately.</p>
