...
class ReceiveNkn extends Notification
{
     ...
     * @return void
     */
    public function __construct($wallet, $amount, $sender)
    {
        $this->wallet = $wallet;
    ...
    {
        if(isset($notifiable->notificationPreferences['receive_nkn'])){
            return $notifiable->notificationPreferences['receive_nkn'];
        } else{
            return [];
        }
    }
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ...
            ->subject('You received some NKN')
            ->line('Your wallet with the address ' . $this->wallet->address .' received some NKN.')
            ->line('')
            ->line('Amount: ' . $this->amount)
            ->line('From: ' . $this->sender)
            ->line('')
            ->line('Don\'t spend it all at once ;)');
    }
    ...
    {
        return [
            'header' => "You received some NKN",
            'excerpt' => "Your address " . $this->wallet->address ." received some NKN.",
            'text' => "
                ...
                <p>Your wallet with the address <b>" . $this->wallet->address ."</b> received some NKN.</p>
