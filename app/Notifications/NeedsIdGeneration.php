...
class NeedsIdGeneration extends Notification
{
    ...
    {
        if(isset($notifiable->notificationPreferences['needs_id_generation'])){
            return $notifiable->notificationPreferences['needs_id_generation'];
        } else{
            return [];
        }
    }
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ...
            ->subject('Your Node needs an ID')
            ->line('Your node with the IP ' . $this->node->addr .' is ready to start earning NKN, it just needs to generate an ID to join the NKN network.')
            ->action('View my nodes', url('/nodes'))
            ->line('If you haven\'t already done please send 10.01 NKN mainnet tokens to the nodes wallet address. This is a fee set by NKN itself to avoid malicious attacks. If you haven\'t got mainnet tokens at hand feel free to use our Node ID generation service right from NKNx.');
    }
    ...
    {
        return [
            'header' => "Your Node needs an ID!",
            'excerpt' => "Your node with the IP " . $this->node->addr . " needs an ID to get into mining.",
            'text' => "
                ...
                <p>Your node with the IP <b>" . $this->node->addr ."</b> is ready to start earning NKN, it just needs to generate an ID to join the NKN network.</p>
                <p>If you haven't already done please send 10.01 NKN mainnet tokens to the nodes wallet address. This is a fee set by NKN itself to avoid malicious attacks. If you haven't got mainnet tokens at hand feel free to use our Node ID generation service right from NKNx.</p>
